...
import type { IAreaRect } from '~/utils/getValidRectFromArea';
import { getValidRectFromArea } from '~/utils/getValidRectFromArea';
  ...
  const imageBytes = await new Promise<Uint8ClampedArray>((resolve, reject) => {
    ...
    const loadCallback = (): void => {
      ...
      const areaRect = image.dataset.area
        ? getValidRectFromArea(image.dataset.area)
        : undefined;
