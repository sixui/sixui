...
import type { IAreaRect } from '~/utils/getValidRectFromSvgArea';
import { getValidRectFromSvgArea } from '~/utils/getValidRectFromSvgArea';
...
export const extractPaletteFromImage = async (
  ...
  const imageBytes = await new Promise<Uint8ClampedArray>((resolve, reject) => {
    ...
    const loadCallback = (): void => {
      ...
      const defaultRect = [0, 0, image.width, image.height] as IAreaRect;
      const areaRect = image.dataset.area
        ? getValidRectFromSvgArea(image.dataset.area)
        : undefined;
